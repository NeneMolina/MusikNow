...
public class CustomListAdapter extends BaseAdapter {
    ...
    public View getView(int pos, View convertView, ViewGroup parent) {
        ...
        //respecto al resultado que obtengamos, mostramos los datos
        try {
            //Mostramos el artista y nombre de la canción
            viewHolder.txtTrackName.setText(result.getTrackName());
            viewHolder.txtArtistName.setText(result.getArtistName());
            //usamos dependencia de Picasso para una vez que coloquemos la direccion de la cancion nos la descarga sin importar la extension
            Picasso.get().load(result.getArtworkUrl100()).error(R.drawable.musica).into(viewHolder.imgPhoto);
            //Aqui almacenamos el cache de las canciones con el id de la cancion y el formato que tiene
            String destFilename = context.getCacheDir() + "/" + result.getTrackId() + ".m4a";
            //preguntamos si la cancion ya existe en los archivos, si ya existe se le da play, sino existe de debe desacrgar
    ...
    //creamos una clase estatica para inicalizar la foto de la caratula, nombre de la cancion, nombre del artista e imagen de estado
